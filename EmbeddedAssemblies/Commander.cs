...
namespace Cradle
{
    ...
    internal sealed class Commander : Cmd
    {
        ...
        ::: Dynamic CS REPL :::
            Usage:      csrepl [dyncs|pydlr]
            Example:    csrepl

            Instructions:

            1. Enter `using` import directives at prompt (directive>), followed by secondary lines (directive..>) until done.
                When done enter `END` on a separate line`.
            2. Enter code at prompt (code>), followed by secondary lines (code..>) until done.
                When done enter `END` on a separate line to kick off compilation/evaluation of entered code.
